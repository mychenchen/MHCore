...
namespace Currency.Common.Caching
{
    public partial class MemoryCacheManager : ILocker, IStaticCacheManager
    {
        ...
        public virtual T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            if ((cacheTime ?? FHCachingDefaults.CacheTime) > 0)
                Set(key, result, cacheTime ?? FHCachingDefaults.CacheTime);
        ...
        /// <param name="cacheTime">Cache time in minutes; pass 0 to do not cache; pass null to use the default time</param>
        /// <returns>The cached value associated with the specified key</returns>
        public virtual T Get<T>(string key, Func<T> acquire, TimeSpan cacheTime)
        {
            //item already is in cache, so return it
            if (_cache.TryGetValue(key, out T value))
                return value;
